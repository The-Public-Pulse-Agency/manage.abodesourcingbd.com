...
export function SampleLedger({ inRows, outRows, perArt, canEdit }: { inRows: MovementRow[]; outRows: MovementRow[]; perArt: ArtRow[]; canEdit: boolean }) {
  const router = useRouter();
  const txt = (v: string) => (v ? v : "—");

  return (
    <div className="space-y-8">
      <SampleInSection rows={inRows} canEdit={canEdit} router={router} txt={txt} />
      <SampleOutSection rows={outRows} canEdit={canEdit} router={router} txt={txt} />
...
function CreateForm({ id, direction, canEdit, router, label, children }: { id: string; direction: "IN" | "OUT"; canEdit: boolean; router: Router; label: string; children: React.ReactNode }) {
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  if (!canEdit) return null;
  return (
    <form
      action={async (fd) => { setBusy(true); const r = await createMovementAction(fd); setBusy(false); if (r.error) setErr(r.error); else { setErr(null); router.refresh(); (document.getElementById(id) as HTMLFormElement)?.reset(); } }}
      id={id}
...
function SampleInSection({ rows, canEdit, router, txt }: { rows: MovementRow[]; canEdit: boolean; router: Router; txt: (v: string) => string }) {
  const cols = 10 + (canEdit ? 1 : 0);
  return (
    <section className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">Sample IN</h2>
      <CreateForm id="sample-in-add" direction="IN" canEdit={canEdit} router={router} label="+ Add sample in">
        ...
        <input name="poNumber" placeholder="PO number" className="input text-sm" aria-label="PO number" />
        <input name="buyer" placeholder="Buyer" className="input text-sm" aria-label="Buyer" />
        <input name="artNo" required placeholder="Art/Style no" className="input text-sm" aria-label="Art/Style no" />
        ...
        <table className="list-table w-full whitespace-nowrap text-sm">
          <thead>
            <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-ink-soft">
              <th className="px-3 py-2.5 font-semibold">Date Received</th><th className="px-3 py-2.5 font-semibold">Sample Type</th><th className="px-3 py-2.5 font-semibold">Qty</th>
              <th className="px-3 py-2.5 font-semibold">PO Number</th><th className="px-3 py-2.5 font-semibold">Buyer</th><th className="px-3 py-2.5 font-semibold">Art/Style No</th>
...
function SampleOutSection({ rows, canEdit, router, txt }: { rows: MovementRow[]; canEdit: boolean; router: Router; txt: (v: string) => string }) {
  const cols = 9 + (canEdit ? 1 : 0);
  return (
    <section className="space-y-3">
      <h2 className="text-lg font-semibold tracking-tight">Sample OUT</h2>
      <CreateForm id="sample-out-add" direction="OUT" canEdit={canEdit} router={router} label="+ Add sample out">
