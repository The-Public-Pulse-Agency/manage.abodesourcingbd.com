...
import { createStyle, createStyleSchema, updateStyle, updateStyleSchema } from "./style";
...
export async function updateStyleFromForm(
  id: string,
  formData: FormData,
): Promise<{ ok: true } | { ok: false; error: string }> {
  const actor = await getCurrentUser();
  if (!actor) return { ok: false, error: "Not authenticated" };
  const parsed = updateStyleSchema.safeParse({
    brandId: formData.get("brandId"),
    styleCode: formData.get("styleCode"),
    name: formData.get("name"),
    category: formData.get("category") || undefined,
  ...
  if (!parsed.success) return { ok: false, error: parsed.error.issues[0].message };
  try {
    await updateStyle(actor, id, parsed.data);
    revalidatePath("/master-data/styles");
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "Failed" };
