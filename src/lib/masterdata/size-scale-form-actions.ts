...
import {
  createSizeScale,
  createSizeScaleSchema,
  updateSizeScale,
  updateSizeScaleSchema,
} from "./sizescale";
...
export async function updateSizeScaleFromForm(
  id: string,
  formData: FormData,
): Promise<{ ok: true } | { ok: false; error: string }> {
  const actor = await getCurrentUser();
  ...
  const sizes = rawSizes
    ...
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
  const parsed = updateSizeScaleSchema.safeParse({
    name: formData.get("name"),
    sizes,
  });
  if (!parsed.success) return { ok: false, error: parsed.error.issues[0].message };
  try {
    await updateSizeScale(actor, id, parsed.data);
    revalidatePath("/master-data/size-scales");
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "Failed" };
