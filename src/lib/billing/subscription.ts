...
import { ForbiddenError, tenantId, type SessionUser } from "@/lib/auth/guard";
import { initiateEps, verifyEps, isPaymentBlocked, epsConfigured, type InitiateResult } from "@/lib/eps";
import type { Subscription } from "@prisma/client";

const DAY = 86_400_000;

/** Only a company ADMIN manages/pays their own licence. */
...
/**
 * Per-company subscription, keyed by companyId (Subscription.id == companyId).
 * Lazily created as a 30-day trial on first read.
 */
export async function getSubscription(companyId: string, opts: { now?: Date } = {}): Promise<Subscription> {
  const now = opts.now ?? new Date();
  return prisma.subscription.upsert({
    where: { id: companyId },
    update: {},
    create: { id: companyId, currentPeriodEnd: new Date(now.getTime() + 30 * DAY) },
  });
}
...
export async function listSubscriptionPayments(companyId: string) {
...
/** Admin edits their company plan — amount/period/name/notes/margin floor are dynamic. */
export async function updatePlan(
  ...
  await getSubscription(companyId);
  ...
  return prisma.subscription.update({ where: { id: companyId }, data });
}
...
export async function startRenewal(actor: SessionUser, customerEmail: string): Promise<StartRenewalResult> {
  ...
  const orderId = `PULSE-SUB-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`.toUpperCase();
...
export async function settleRenewal(orderId: string): Promise<SettleResult> {
  ...
  const companyId = pay.subscriptionId; // subscription id == companyId
  const sub = await getSubscription(companyId);
  const now = new Date();
  ...
  await prisma.$transaction([
    ...
    prisma.subscription.update({ where: { id: companyId }, data: { status: "ACTIVE", currentPeriodEnd: newEnd, lastPaymentAt: now } }),
  ]);
  return "paid";
}
