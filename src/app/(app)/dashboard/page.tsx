...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-sm border border-line bg-line lg:grid-cols-5">
  ...
  return (
    <div className="overflow-hidden rounded-sm border border-line bg-surface">
      <div className="flex items-center justify-between border-b border-line bg-paper px-4 py-2">
        ...
        <Link href={href} className="text-xs text-ink-soft hover:text-accent">{linkLabel} →</Link>
      </div>
      {children}
    </div>
  );
...
function Exception({ label, count, href, tone }: { label: string; count: number; href: string; tone: "bad" | "warn" }) {
  ...
  return (
    <Link href={href} className="group bg-surface p-4 transition-colors hover:bg-paper">
      <p className="eyebrow">{label}</p>
